	...
	init = function(self)
		...
		self.cell = Cell{
			...
			execute = function(cell)
                ...
                -- Mutation 1: Relational Operator Replacement (ROR2: >= => <=)
                -- if cell.past.lifeSpan >= self.lifeSpan then ... ORIG
				...
				if cell.past.daisy == "empty" and cell.past.soilHeat > self.reproduceTemperature.min and cell.past.soilHeat< self.reproduceTemperature.max then
					...
					-- if self.rand:number() * 100 < probabilityReproduce then ... ORIG
...
for i=1,120 do
  ...
  scene.map2:save("DaisyWorld_mutant_" .. value .. i .. ".png")  
