...
import torch
import os
from utilsmetricsalt import *
from utilsioalt import *
from torchvision import models
import torch.nn as nn
import torch.nn.functional as F

def get_data_CNNIn(op, pre):
  model_e = []
...
class ModelExtFeaturesIn1L(nn.Module):
    def __init__(self, option, pre_tr, n_ext, nc):
        super(ModelExtFeaturesIn1L, self).__init__()
        self.cnn, self.num_ftrs = get_data_CNNIn(option, pre_tr)
        if option != '7': 
...
class ModelExtFeaturesIn2L(nn.Module):
    def __init__(self, option, pre_tr, n_ext, nc):
        super(ModelExtFeaturesIn2L, self).__init__()
        self.cnn, self.num_ftrs = get_data_CNNIn(option, pre_tr)
        if option != '7': 
          self.cnn.fc = nn.Linear(
              self.num_ftrs, self.num_ftrs)
...
def train_model(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, device, fname):
    since = time.time()
    # Get user options
    output_dir, best_model_full, indic, early_prop, num_epochs = get_user_options(fname)
    ...
    while (epoch < num_epochs) and (no_improvements < early_stopping):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels, paths in dataloaders[phase]:
                ...
                running_corr_cpu = running_corrects.cpu().detach().numpy()
                if (count == 1):
            ...
            epoch_acc = running_corr_cpu / dataset_sizes[phase]
...
# Inference phase
def test_model(mod_opt, num_ftrs, num_classes, crit, dataloaders, dataset_sizes, device, fname): 
    since = time.time()
    # Get user options
    output_dir, bm, indic, early_prop, num_epochs = get_user_options(fname)
    # Load the best model
    if mod_opt =='1':
        model = models.resnet18()
        print('Inference, Input features - ResNet-18:', num_ftrs) 
        model.fc = nn.Linear(num_ftrs, num_classes)
    elif mod_opt =='2':
        model = models.resnet34()
        print('Inference, Input features - ResNet-34:', num_ftrs)
        model.fc = nn.Linear(num_ftrs, num_classes)
    elif mod_opt =='3':
        model = models.resnext50_32x4d()
        print('Inference, Input - features ResNeXt-50-32x4d:', num_ftrs)
        model.fc = nn.Linear(num_ftrs, num_classes)
    elif mod_opt =='4':
        model = models.wide_resnet50_2()
        print('Inference, Input features - Wide ResNet-50-2:', num_ftrs)
        model.fc = nn.Linear(num_ftrs, num_classes)
    elif mod_opt =='5':
        ...
        print('Inference, Input features - ResNet-152:', num_ftrs)
        model.fc = nn.Linear(num_ftrs, num_classes)
    elif mod_opt =='7':
        model = models.densenet161()
        print('Inference, Input features - DeseNet-161:', num_ftrs)
...
# Training phase (training and validation)
def train_model_ext(model, criterion, optimizer, scheduler, dataloaders, dataset_sizes, device, class_names, fname):
    since = time.time()
    # Get user options
    output_dir, best_model_full, indic, extf_dir, n_ext, bs, early_prop, num_epochs = get_user_options_ext(fname)
    # Dictionaries: truth and preds values
    d_truth = {}
    d_preds = {}
    # Auxiliary variables: training and validation losses and accuracy; 
...
# Inference phase
def test_model_ext(mod_opt, num_ftrs, num_classes, crit, dataloaders, dataset_sizes, device, class_names, fname):
    since = time.time()
    # Get user options
    output_dir, bm, indic, extf_dir, n_ext, bs, early_prop, num_epochs = get_user_options_ext(fname)
    # Load the best model
    if (fname == 'user_options1t') or (fname == 'user_options1s'): #TLE1L
        model = ModelExtFeaturesIn1L(mod_opt, False, n_ext, num_classes)  
    elif (fname == 'user_options2t') or (fname == 'user_options2s'): #TLE2L 
        model = ModelExtFeaturesIn2L(mod_opt, False, n_ext, num_classes)  
    else:
        print('Invalid option!')
