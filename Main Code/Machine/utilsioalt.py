...
def save_user_options(out_dir, best_model, ind, early_prop, n_epoch, fname):
    f = open(fname+'.txt',"w+")
    f.write(out_dir+'\n')
    f.write(best_model+'\n')
    f.write(ind+'\n')
...
def save_user_options_ext(out_dir, best_model, ind, extf_dir, n_ext, bs, early_prop, n_epoch, fname):
    f = open(fname+'.txt',"w+")
    f.write(out_dir+'\n')
    f.write(best_model+'\n')
    f.write(ind+'\n')
...
def get_user_options_ext(fname):
    with open(fname+'.txt') as f:
        j = 1
        for line in f:
            if j == 1:
...
def plot_training_metrics(all_losses_acc, d_precision, d_recall, d_f1, fname):
    
    output_dir, best_model, indic, early_prop, num_epochs = get_user_options(fname)
...
def plot_training_metrics_ext(all_losses_acc, d_precision, d_recall, d_f1, fname):
    
    output_dir, best_model, indic, extf_dir, n_ext, bs, early_prop, num_epochs = get_user_options_ext(fname)
    x = range(1,len(d_precision['train'])+1)
